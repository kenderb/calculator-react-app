...
describe('Calculate method', () => {
  ...
  it('Should acept and object and a name as arguments btnName = -', () => {
    expect(calculate({ total: 30, next: 2, operation: '-' })).toEqual({ next: 0, operation: '-' });
  });
  it('Should acept and object and a name as arguments btnName = X', () => {
    expect(calculate({ total: 30, next: 2, operation: 'X' })).toEqual({ next: 0, operation: 'X' });
  });
  it('Should acept and object and a name as arguments btnName = +', () => {
    expect(calculate({ total: 30, next: 2, operation: '+' })).toEqual({ next: 0, operation: '+' });
  });
  it('Should acept and object and a name as arguments btnName = %', () => {
    expect(calculate({ total: 30, next: 2, operation: '%' })).toEqual({ total: 0.30, next: '=', operation: null });
  });
  it('Should acept and object and a name as arguments btnName = ÷', () => {
    expect(calculate({ total: 30, next: 2, operation: '÷' })).toEqual({ next: 0, operation: '÷' });
  });
