...
describe('Calculate method', () => {
  ...
  it('Should accept and object and a name as arguments btnName = -', () => {
    expect(calculate({ total: 30, next: 2, operation: '-' })).toEqual({ next: '0', operation: '-', total: 30 });
  });
  it('Should accept and object and a name as arguments btnName = X', () => {
  ...
  it('Should accept and object and a name as arguments btnName = e', () => {
    expect(calculate({ total: 30, next: 2, operation: 'e' })).toEqual({ next: null, total: '30e' });
  });

  it('should use the buttonName argument to mutate the calculator’s data object with a negative value', () => {
    expect(calculate({ total: 30, next: -1, operation: '+/-' })).toEqual({ total: '-30' });
  });
  it('should use the buttonName argument to mutate the calculator’s data object with a positive value.', () => {
    expect(calculate({ total: 30, next: 1, operation: '+/-' })).toEqual({ total: '-30' });
  });
});
